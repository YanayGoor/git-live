...
Node._fields_ = [
    ("entry", NodeListEntry),
    ("basis", ctypes.c_size_t),
    ("expand", ctypes.c_size_t),
    ("fit_content", ctypes.c_bool),
    ("wrap", ctypes.c_int),
    ("padding_top", ctypes.c_size_t),
    ("padding_bottom", ctypes.c_size_t),
    ("padding_left", ctypes.c_size_t),
    ("padding_right", ctypes.c_size_t),
...
class Library:
    ...
    def init_layout(self, scr: Screen) -> tuple[ctypes.c_void_p, NodePointer]:
        ...
        assert not self._library.get_layout_root(layout, ctypes.byref(root))

        return layout, root

    def append_child(self, parent: NodePointer) -> NodePointer:
        child = ctypes.POINTER(Node)()
        assert not self._library.append_child(parent, ctypes.byref(child))
        return child

    def append_text(self, parent: NodePointer, text: bytes) -> None:
        assert not self._library.append_text(parent, text)

    def append_styled_text(
        self, parent: NodePointer, text: bytes, color: int, attrs: int
    ) -> None:
        assert not self._library.append_styled_text(parent, text, color, attrs)

    def draw_layout(self, layout: Layout, scr: Screen) -> None:
        assert not self._library.draw_layout(layout, Rect(0, 0, scr.width, scr.height))
